...
@ExtendWith(MockitoExtension::class)
class BookingServiceTest_RecurringComplete {
    ...
    private lateinit var service: Services
    private lateinit var slot: ScheduleSlot
    private lateinit var recurringBooking: Booking
    ...
    @BeforeEach
    fun setUp() {
        seeker = ServiceSeeker("seeker", "pass", "mail", "Seeker", "", "Skopje").apply { id = 1L }
        provider = ServiceProvider("provider", "pass", "mail", "Provider", "", "Skopje").apply { id = 2L }
        val category = ServiceCategory(1L, "Cleaning")
        service = Services(
            id = 1L,
            title = "Cleaning",
            description = "desc",
    ...
    @Test
    fun `completeBooking should mark as completed and create new recurring booking`() {
        val bookingId = recurringBooking.id!!
        val completedBooking = recurringBooking.copy(status = BookingStatus.COMPLETED)
        ...
        `when`(bookingRepository.findById(bookingId)).thenReturn(Optional.of(recurringBooking))
        ...
        // verify recurring booking was created
        verify(bookingRepository, times(2)).save(bookingCaptor.capture())
        val savedBookings = bookingCaptor.allValues
        assertTrue(savedBookings.any { it.status == BookingStatus.COMPLETED })
        assertTrue(savedBookings.any { it.isRecurring && it.status == BookingStatus.PENDING })
        assertEquals(newSlot.slot_id, savedBookings[1].slot.slot_id)
        assertEquals(Status.AVAILABLE, savedBookings[1].slot.status)
    }
}
